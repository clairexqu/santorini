...
class Player:
    ...
    def build_valid_placements(self, board):
        ...
        for worker in self._own_workers:
            ...
            for (
            ...
            ) in DIRECTION_TRANSFORMATION.items():
                ...
                if self._valid_placement(
                    current_worker_coordinate, turn.placement_coordinate, board
                ):
    ...
    def _calc_score(self, board, turn):
        ...
        for coordinate in coordinates:
            move_score.height_score += self._calculate_height_score(
                board, coordinate, turn.move_score
            )
        ...
        for enemy_coordinate in enemy_coordinates:
            distance_before_eight += self._calculate_distance_score(
                enemy_coordinate, coordinates
            )
    ...
    def _calculate_center_score(self, coordinate):
        ...
        elif (row == 1 or row == 2 or row == 3) and (
...
class HumanPlayer(Player):
    ...
    def _get_valid_turn(self, worker, potential_placement):
        ...
        for turn in self._valid_placements:
            if (
                turn.worker == worker
                and turn.placement_direction == potential_placement
            ):
                return turn
        return None
